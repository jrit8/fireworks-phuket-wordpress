<?php defined( 'ABSPATH' ) || exit; $status = isset( $_GET['inquiry'] ) && is_string( $_GET['inquiry'] ) ? sanitize_key( wp_unslash( $_GET['inquiry'] ) ) : ''; ?>
<section class="section"><div class="container quote-grid" id="quote-form"><div><p class="eyebrow">Request a quote</p><h2>Tell us about your celebration.</h2><p class="copy">Share what you know so far. We’ll recommend the best direction based on your date, venue and plans.</p><p>Phuket · Khao Lak · Krabi</p><?php if ( get_theme_mod( 'fp_whatsapp', '' ) ) : ?><a class="text-link" href="<?php echo esc_url( fp_contact_url() ); ?>">Prefer WhatsApp? Message us directly →</a><?php endif; ?></div><div>
...
<label>Event type<input name="event" maxlength="300"></label>
